import { createHttpClient } from "@clean-chat/client";
import { UNKNOWN_AUTHOR_NAME } from "@clean-chat/core/domain";
import { afterEach, describe, expect, it } from "vitest";
import { createServer } from "../src/server.js";
import { createSqlPersistence } from "../src/sql/index.js";

const user = {
  id: "user-1",
  email: "ada@example.com",
  name: "Ada",
};

const general = {
  id: "ch-g",
  name: "general",
  createdBy: user.id,
};

describe("sql persistence", () => {
  const dbs: Array<{ close(): void }> = [];

  afterEach(() => {
    for (const db of dbs.splice(0)) {
      db.close();
    }
  });

  async function openSql() {
    const persistence = await createSqlPersistence();
    dbs.push(persistence);
    return persistence;
  }

  it("stores channels and looks them up by name", async () => {
    const { uow, channels } = await openSql();
    await uow.run(async (tx) => {
      await channels.insert(tx, general);
    });
    await uow.run(async (tx) => {
      await expect(channels.getByName(tx, "general")).resolves.toEqual(general);
      await expect(channels.getById(tx, general.id)).resolves.toEqual(general);
      await expect(channels.list(tx)).resolves.toEqual([general]);
    });
  });

  it("joins authorName on message read, Unknown if the user is gone", async () => {
    const { uow, messages, users, authUsers } = await openSql();
    await authUsers.put(user);
    await uow.run(async (tx) => {
      await expect(users.getById(tx, user.id)).resolves.toEqual(user);
      await messages.insert(tx, {
        id: "m1",
        channelId: "ch-1",
        authorId: user.id,
        body: "hello",
        createdAt: 10,
      });
      await messages.insert(tx, {
        id: "m2",
        channelId: "ch-1",
        authorId: "gone",
        body: "bye",
        createdAt: 20,
      });
    });
    const listed = await uow.run((tx) =>
      messages.listLatestByChannel(tx, "ch-1", 50),
    );
    expect(listed.map((m) => m.authorName)).toEqual([
      user.name,
      UNKNOWN_AUTHOR_NAME,
    ]);
    expect(listed.map((m) => m.id)).toEqual(["m1", "m2"]);
    await uow.run(async (tx) => {
      await expect(messages.getById(tx, "m2")).resolves.toMatchObject({
        authorName: UNKNOWN_AUTHOR_NAME,
      });
    });
  });

  it("returns the latest N messages oldest-first, newest id when times tie", async () => {
    const { uow, messages } = await openSql();
    await uow.run(async (tx) => {
      await messages.insert(tx, {
        id: "m1",
        channelId: "ch-1",
        authorId: user.id,
        body: "a",
        createdAt: 1,
      });
      await messages.insert(tx, {
        id: "m2",
        channelId: "ch-1",
        authorId: user.id,
        body: "b",
        createdAt: 2,
      });
      await messages.insert(tx, {
        id: "m3",
        channelId: "ch-1",
        authorId: user.id,
        body: "c",
        createdAt: 3,
      });
      await messages.insert(tx, {
        id: "m4",
        channelId: "ch-1",
        authorId: user.id,
        body: "d",
        createdAt: 3,
      });
    });
    const listed = await uow.run((tx) =>
      messages.listLatestByChannel(tx, "ch-1", 2),
    );
    expect(listed.map((m) => m.id)).toEqual(["m3", "m4"]);
  });

  it("removes a message by id", async () => {
    const { uow, messages } = await openSql();
    await uow.run(async (tx) => {
      await messages.insert(tx, {
        id: "m1",
        channelId: "ch-1",
        authorId: user.id,
        body: "a",
        createdAt: 1,
      });
      await messages.remove(tx, "m1");
    });
    await uow.run(async (tx) => {
      await expect(messages.getById(tx, "m1")).resolves.toBeNull();
    });
  });

  it("upserts typing by (channelId, userId)", async () => {
    const { uow, typing } = await openSql();
    await uow.run(async (tx) => {
      await typing.put(tx, {
        channelId: "ch-1",
        userId: user.id,
        name: "Ada",
        updatedAt: 1,
      });
      await typing.put(tx, {
        channelId: "ch-1",
        userId: user.id,
        name: "Ada",
        updatedAt: 2,
      });
    });
    const rows = await uow.run((tx) => typing.listByChannel(tx, "ch-1"));
    expect(rows).toHaveLength(1);
    expect(rows[0]?.updatedAt).toBe(2);
  });

  it("keys presence by (channelId, sessionId)", async () => {
    const { uow, presence } = await openSql();
    const row = {
      channelId: "ch-1",
      userId: user.id,
      sessionId: "sess-1",
      online: true,
      name: user.name,
    };
    await uow.run(async (tx) => {
      await presence.put(tx, row);
    });
    await uow.run(async (tx) => {
      await expect(presence.get(tx, "ch-1", "sess-1")).resolves.toEqual(row);
      await expect(presence.listByChannel(tx, "ch-1")).resolves.toEqual([row]);
      await presence.remove(tx, "ch-1", "sess-1");
      await expect(presence.get(tx, "ch-1", "sess-1")).resolves.toBeNull();
    });
  });

  it("drops a session from every channel except the one it heartbeats", async () => {
    const { uow, presence } = await openSql();
    const sess = "sess-1";
    await uow.run(async (tx) => {
      await presence.put(tx, {
        channelId: "ch-1",
        userId: user.id,
        sessionId: sess,
        online: true,
        name: user.name,
      });
      await presence.put(tx, {
        channelId: "ch-2",
        userId: user.id,
        sessionId: sess,
        online: true,
        name: user.name,
      });
      await presence.put(tx, {
        channelId: "ch-3",
        userId: user.id,
        sessionId: sess,
        online: false,
        name: user.name,
      });
    });
    const left = await uow.run((tx) =>
      presence.removeSessionFromOtherChannels(tx, sess, "ch-2"),
    );
    expect(left).toEqual(["ch-1"]);
    await uow.run(async (tx) => {
      await expect(presence.get(tx, "ch-1", sess)).resolves.toBeNull();
      await expect(presence.get(tx, "ch-3", sess)).resolves.toBeNull();
      await expect(presence.get(tx, "ch-2", sess)).resolves.toMatchObject({
        channelId: "ch-2",
      });
    });
  });

  it("rolls the store back when UnitOfWork.run throws", async () => {
    const { uow, channels } = await openSql();
    await expect(
      uow.run(async (tx) => {
        await channels.insert(tx, general);
        throw new Error("boom");
      }),
    ).rejects.toThrow("boom");
    await uow.run(async (tx) => {
      await expect(channels.list(tx)).resolves.toEqual([]);
    });
  });

  it("wires createServer so sign-up users join message authors", async () => {
    const persistence = await openSql();
    const server = createServer({ port: 0, persistence });
    await server.start();
    try {
      const client = createHttpClient({
        baseUrl: `http://${server.host}:${String(server.port)}`,
      });
      const ada = await client.signUp.execute({
        email: "ada@example.com",
        password: "password1",
        name: "Ada",
      });
      const channelId = await client.ensureGeneralChannel.execute();
      await client.sendMessage.execute({ channelId, body: "hello" });
      const listed = await client.listMessages.execute({ channelId });
      expect(listed).toEqual([
        expect.objectContaining({
          authorId: ada.id,
          authorName: "Ada",
          body: "hello",
        }),
      ]);
    } finally {
      await server.stop();
    }
  });
});
