...
- Composition root: `createServer` in `@clean-chat/infrastructure` (`src/server.ts`); `src/main.ts` listens. `POST /use-cases/{name}`, `GET /events` (SSE, all types; per-type `GET /events/{type}` still exists). `CORS_ORIGIN` (default `*` on loopback, or a comma-separated allowlist). Off-loopback binds require an explicit allowlist
- UI: Vite + React 19 + Tailwind 4 + shadcn/ui (`base-nova`) in `@clean-chat/web`. No client-side URL router. Session is `Authorization: Bearer` (not cookies/JWT library). Cookie flags / CSRF are skipped because there are no cookies. `npm run dev` runs Express (`127.0.0.1:3000`) and Vite (`localhost:5173`). `VITE_API_URL` defaults to `http://127.0.0.1:3000`.
