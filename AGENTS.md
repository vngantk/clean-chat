...
- Packages: `core` → `application` → `infrastructure`; `client` → `core`
...
- HTTP server: Express in `@clean-chat/infrastructure` (`src/http/`). `createExpressUseCaseRouter` mounts `POST /{useCaseName}` → `UseCase.execute`. `createExpressEventSubscriptionRouter` mounts `GET /{eventType}` SSE via `EventSubscriber`. `createExpressServer` takes a path→router map, `port`, optional `host`, optional `corsOrigins` (`*` on loopback only), and returns a `Lifecycle`. JSON / **204** for use cases (`void` → 204 No Content). Known errors are 401 / 403 / 400 `{ error }`; unexpected errors are 500 without a stack. SSE requires a bearer session. Not a public REST API
